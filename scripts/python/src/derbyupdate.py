...
for index, line in enumerate(file):
	print "inside for index" + commit_id
	line1 = line.replace(" ","")
		...
		while (bool_flag < 1):
			try:
				...
				if "IN_PROGRESS" in buildStatusJson["stages"][i]["status"]:
					...
					resulttemp=subprocess.check_output(['java', '-jar', 'derbyUpdate.jar',commit_id,"INPROGRESS","0",now.strftime("%Y-%m-%d"),cjob,now.strftime("%Y-%m-%d %H:%M:%S"),time.strftime('%Y-%m-%d %H:%M:%S',  time.gmtime(int(buildStatusJson["stages"][i]["startTimeMillis"])/1000.))])
				else:
					now = datetime.datetime.now()
			                resulttemp=subprocess.check_output(['java', '-jar', 'derbyUpdate.jar',commit_id,buildStatusJson["stages"][i]["status"],str(buildStatusJson["stages"][i]["durationMillis"]),now.strftime("%Y-%m-%d"),cjob,now.strftime("%Y-%m-%d %H:%M:%S"),time.strftime('%Y-%m-%d %H:%M:%S',  time.gmtime(int(buildStatusJson["stages"][i]["startTimeMillis"])/1000.))])
					bool_flag = 1
					i = i + 1
					if "FAILURE" in buildStatusJson["stages"][i]["status"]:
						failureflag = 'true'
