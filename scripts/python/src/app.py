...
def setup():
    ...
    print data
    #print "data value ends"
    build = BuildOn(data)
    buildon = build.setup(data)
    print buildon
    if (buildon != 0):
	...
	pid = subprocess.Popen([sys.executable, "/home/ubuntu/BuildOn/derbyupdate.py"])
    #subprocess.check_output("kubectl get pods --show-all |grep -e Error -e Completed | cut -f1 -d ' '| xargs kubectl delete pods ",shell=True)
    return "ok"
